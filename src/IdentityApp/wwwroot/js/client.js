...
function createAuthPromt(targetElement) {
    let signedIn = false;
    ...
    button.addEventListener("click", async () => {
        if (!signedIn) {
            await network.signIn("alice_auto@example.com", "MySecret1$", response => {
...
function createTableContents(products) {
    ...
    products.forEach(p => {
        ...
        const button = row.appendChild(document.createElement("td")).appendChild(document.createElement("button"));
